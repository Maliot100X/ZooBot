#!/usr/bin/env bash
# tc-api.sh — Thin wrapper for ZooBot API operations
...
check_api() {
    if ! curl -sf "${API_BASE}/api/queue/status" > /dev/null 2>&1; then
        echo "ERROR: ZooBot API not reachable at ${API_BASE}" >&2
        echo "Is ZooBot running? Try: zoobot start" >&2
        exit 1
    fi
}
