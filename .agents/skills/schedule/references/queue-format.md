...
Scheduled tasks deliver messages by POSTing to the ZooBot API server (`POST /api/message`).

## API endpoint

```
POST http://localhost:{ZOOBOT_API_PORT}/api/message
...
```json
{
  "channel": "schedule",
  "sender": "Scheduler",
  "senderId": "zoobot-schedule:<label>",
...
| `senderId`  | string | Unique sender ID. Format: `zoobot-schedule:<label>`. |
