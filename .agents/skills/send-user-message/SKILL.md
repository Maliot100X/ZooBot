...
Send a message to a paired user via the ZooBot API server (`POST /api/responses`). The message is delivered by the channel client (Discord, Telegram, or WhatsApp) that polls the API for pending responses.
...
<skill_dir>/scripts/send-message.sh send \
  ...
  --files "/Users/you/.zoobot/files/report.pdf,/Users/you/.zoobot/files/chart.png"
```
...
- The script POSTs to the API server (default `http://localhost:3777`), configurable via `ZOOBOT_API_PORT` env var
