...
ZooBot supports plugins that can intercept messages, transform content, and react to system events. Plugins are auto-discovered from the `plugins/` folder inside your ZooBot home directory.
...
```bash
mkdir -p ~/.zoobot/plugins/my-plugin
```
...
3. Restart ZooBot. Your plugin loads automatically.
...
Returns the resolved ZooBot home directory path (e.g., `~/.zoobot`).
...
exports.activate = function(ctx) {
  const logFile = path.join(ctx.getZooBotHome(), 'plugins', 'logger', 'messages.log');
...
```bash
cd ~/.zoobot/plugins/my-plugin
npx tsc index.ts --outDir . --skipLibCheck
```

The plugin loader will pick up `index.js`. Types can be imported from the ZooBot source if you have it available:

```ts
import type { PluginContext, Hooks, HookContext } from 'zoobot/src/lib/plugins';
```
