...
### Highlights
- **Zero-Config Onboarding** — Completely streamlined the first-run experience. Running `zoobot` now auto-creates default settings, bootstraps agent workspace directories, checks dependencies, and launches the daemon — no setup wizard required. Users can add channels later via the web UI or `zoobot channel setup`. (8e5a1b9)
...
- `zoobot channel setup` command for interactive channel token configuration, replacing the removed setup wizard (8e5a1b9)
...
- Office web UI automatically opens on `zoobot`, `zoobot start`, and `zoobot restart` (8e5a1b9)
...
### Improvements
- Default agent renamed from `default` to `zoobot` across all routing, config, and fallback logic (8e5a1b9)
...
- CLI prefers local repo `zoobot.sh` over installed copy when running from source (dev workflow) (8e5a1b9)
...
### Breaking Changes
- Setup wizard (`setup-wizard.ts`) and `/setup` web page have been removed. Use `zoobot channel setup` for channel configuration instead (8e5a1b9)
...
- Default agent ID changed from `default` to `zoobot`. Existing configurations referencing `default` may need to be updated (8e5a1b9)
...
- 8e5a1b9 refactor(cli): streamline onboarding to single 'zoobot' command (#244)
