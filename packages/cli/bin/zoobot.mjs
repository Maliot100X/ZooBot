...
import os from 'os';
import { writeDefaults, ZOOBOT_HOME } from '../lib/defaults.mjs';
...
function isInstalled() {
    ...
    return fs.existsSync(path.join(INSTALL_DIR, 'lib/zoobot.sh'))
        || fs.existsSync(path.join(repoRoot, 'lib/zoobot.sh'));
}

async function install() {
    log(BLUE, 'Installing ZooBot...');
    ...
    try {
        ...
        if (match) {
            const tag = match[1];
            const bundleUrl = `https://github.com/${GITHUB_REPO}/releases/download/${tag}/zoobot-bundle.tar.gz`;
    ...
    exec(`chmod +x "${INSTALL_DIR}/bin/zoobot" "${INSTALL_DIR}/bin/zoobot" "${INSTALL_DIR}/lib/zoobot.sh" "${INSTALL_DIR}/lib/heartbeat-cron.sh" "${INSTALL_DIR}/packages/cli/bin/zoobot.mjs"`);

    // Install CLI symlink (zoobot command)
    installCli();

    log(GREEN, '✓ ZooBot installed');
    console.log('');
}
...
function installCli() {
    ...
    const zoobotSrc = path.join(INSTALL_DIR, 'packages/cli/bin/zoobot.mjs');
    let installDir = '';
    ...
    const symlinkPath = path.join(installDir, 'zoobot');
    ...
    // Create symlink
    fs.symlinkSync(zoobotSrc, symlinkPath);
    log(GREEN, `✓ 'zoobot' command installed at ${symlinkPath}`);
...
async function run() {
    // If settings already exist, just delegate to `zoobot start` + open office
    if (isInstalled() && fs.existsSync(path.join(os.homedir(), '.zoobot', 'settings.json'))) {
    ...
    if (!isInstalled()) {
        ...
        log(GREEN, '✓ ZooBot already installed');
        console.log('');
    }
    ...
    console.log(`  Workspace: ~/zoobot-workspace`);
    console.log(`  Agent: zoobot (anthropic/opus)`);
    console.log('');
    ...
    try {
        ...
        log(YELLOW, 'ZooBot may already be running (use zoobot status to check)');
    }
...
// ── Delegate to bash (zoobot.sh) ───────────────────────────────────────────

function delegateToBash(args, opts = {}) {
    // Prefer local repo copy when running from source (dev workflow)
    const repoRoot = path.resolve(new URL('.', import.meta.url).pathname, '../../..');
    const localSh = path.join(repoRoot, 'lib/zoobot.sh');
    const installedSh = path.join(INSTALL_DIR, 'lib/zoobot.sh');
    const zoobotSh = fs.existsSync(localSh) ? localSh : installedSh;

    if (!fs.existsSync(zoobotSh)) {
        log(RED, 'ZooBot is not installed. Run "zoobot" first.');
        process.exit(1);
    }

    if (opts.sync) {
        execSync(`"${zoobotSh}" ${args.map(a => `"${a}"`).join(' ')}`, { stdio: 'inherit' });
    } else {
        const child = spawn(zoobotSh, args, { stdio: 'inherit' });
...
// Commands that zoobot handles directly
switch (command) {
    case 'run':
        run();
        break;
    ...
    case 'help':
        console.log('');
        console.log('Usage: zoobot [command]');
        console.log('');
        console.log('Quick Start:');
        console.log('  run                      Install, configure defaults, and start (default)');
        console.log('  install                  Install ZooBot only');
        console.log('');
        console.log('Daemon:');
        console.log('  start                    Start ZooBot');
        console.log('  stop                     Stop all processes');
        console.log('  restart                  Restart ZooBot');
        ...
        console.log('  update                   Update ZooBot to latest version');
    ...
    // All other commands delegate to zoobot.sh
    default:
