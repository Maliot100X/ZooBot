...
import { CustomProvider, ensureAgentDirectory } from '@zoobot/core';
import {
...
async function agentAdd() {
    ...
    const workspacePath = settings.workspace?.path || path.join(process.env.HOME || '~', 'zoobot-workspace');
...
async function customProviderAdd() {
    ...
    p.outro(`Assign to an agent: zoobot agent provider <agent_id> custom:${providerId}`);
}
...
function agentList() {
    ...
    if (ids.length === 0) {
        ...
        p.log.message('Add an agent with: zoobot agent add');
        return;
    }
    ...
    switch (providerArg) {
        ...
        default:
            if (providerArg.startsWith('custom:')) {
                ...
                p.log.error('Usage: zoobot agent provider <agent_id> {anthropic|openai|opencode|custom:<id>} [--model MODEL]');
                process.exit(1);
            }
    }
...
function customProviderList() {
    ...
    if (ids.length === 0) {
        ...
        p.log.message('Add one with: zoobot provider add');
        return;
    }
    ...
    p.log.message('  zoobot agent provider <agent_id> custom:<provider_id>');
}
