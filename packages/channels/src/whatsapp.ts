#!/usr/bin/env node
/**
 * WhatsApp Client for ZooBot Simple
...
const LOG_FILE = path.join(ZOOBOT_HOME, 'logs/whatsapp.log');
const SESSION_DIR = path.join(ZOOBOT_HOME, 'whatsapp-session');
...
client.on('qr', (qr: string) => {
    ...
    // Save to file for zoobot.sh to display (avoids tmux capture distortion)
    const channelsDir = path.join(ZOOBOT_HOME, 'channels');
    ...
    qrcode.generate(qr, { small: true }, (code: string) => {
        ...
        log('INFO', 'QR code saved to .zoobot/channels/whatsapp_qr.txt');
    });
...
client.on('ready', () => {
    ...
    // Create ready flag for zoobot.sh
    const readyFile = path.join(ZOOBOT_HOME, 'channels/whatsapp_ready');
...
client.on('message_create', async (message: Message) => {
    try {
        ...
        if (messageText.trim().match(/^[!/]restart$/i)) {
            ...
            await message.reply('Restarting ZooBot...');
