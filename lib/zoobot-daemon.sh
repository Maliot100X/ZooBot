...
start_daemon() {
    ...
    if [ $load_rc -eq 2 ]; then
        ...
        if [ $load_rc -ne 0 ]; then
            ...
            echo "  Or reconfigure: zoobot setup"
            return 1
        fi
    ...
    for ch in "${ACTIVE_CHANNELS[@]}"; do
        ...
        if [ -n "$token_key" ] && [ -z "$(get_channel_token "$ch")" ]; then
            ...
            echo "Run 'zoobot setup' to reconfigure"
            return 1
        fi
    done
    ...
    if [ ${#ACTIVE_CHANNELS[@]} -gt 0 ]; then
        ...
        echo -e "${BLUE}No channels configured.${NC} Add channels later with 'zoobot channel setup'"
        echo ""
    fi
    ...
    tmux new-session -d -s "$TMUX_SESSION" -n "zoobot" -c "$SCRIPT_DIR"
    ...
    # shell and exit silently. See: https://github.com/ZooBot/zoobot/issues/156
    sleep 2
    ...
    echo ""
    echo -e "${GREEN}✓ ZooBot started${NC}"
    echo ""
    ...
    if [ "$whatsapp_pane" -ge 0 ]; then
        ...
        QR_FILE="$ZOOBOT_HOME/channels/whatsapp_qr.txt"
        READY_FILE="$ZOOBOT_HOME/channels/whatsapp_ready"
        QR_DISPLAYED=false
        ...
        if [ $i -eq 60 ] && [ ! -f "$READY_FILE" ]; then
            ...
            echo ""
            echo -e "${YELLOW}Try restarting ZooBot:${NC}"
            echo -e "  ${GREEN}zoobot restart${NC}"
            echo ""
            echo "Or check WhatsApp client status:"
            echo -e "  ${GREEN}tmux attach -t $TMUX_SESSION${NC}"
            echo ""
            echo "Or check logs:"
            echo -e "  ${GREEN}zoobot logs whatsapp${NC}"
            echo ""
        fi
    fi

    echo -e "${GREEN}Commands:${NC}"
    echo "  Status:  zoobot status"
    echo "  Logs:    zoobot logs queue"
...
# Creates a proper tmux session so zoobot stop/restart still work.
_start_server_only() {
    # Ensure ZOOBOT_HOME directories exist so the server can write settings
    mkdir -p "$ZOOBOT_HOME/logs"
    mkdir -p "$ZOOBOT_HOME/files"
    ...
    echo -e "${GREEN}✓ ZooBot started (setup mode — no channels)${NC}"
    echo ""
    echo -e "API server: ${BLUE}http://localhost:${ZOOBOT_API_PORT:-3777}${NC}"
    echo ""
    echo -e "Complete setup in your browser:"
    echo -e "  ${BLUE}http://localhost:3000/setup${NC}  (TinyOffice)"
    echo -e "  or run: ${BLUE}zoobot office${NC}"
    echo ""
    echo -e "Once setup is complete, restart to enable channels:"
    echo -e "  ${BLUE}zoobot restart${NC}"
    echo ""
...
start_channel() {
    local channel="$1"
    if [ -z "$channel" ]; then
        echo -e "${RED}Usage: zoobot channel start <channel_id>${NC}"
        return 1
    fi
    ...
    if ! session_exists; then
        echo -e "${RED}No tmux session running. Start ZooBot first.${NC}"
        return 1
    fi
...
stop_channel() {
    local channel="$1"
    if [ -z "$channel" ]; then
        echo -e "${RED}Usage: zoobot channel stop <channel_id>${NC}"
        return 1
    fi
...
stop_daemon() {
    ...
    echo -e "${GREEN}✓ ZooBot stopped${NC}"
    log "Daemon stopped"
}

# Restart daemon safely even when called from inside ZooBot's tmux session
restart_daemon() {
    if session_exists && [ -n "${TMUX:-}" ]; then
        ...
        if [ "$current_session" = "$TMUX_SESSION" ]; then
            ...
            nohup "$bash_bin" "$SCRIPT_DIR/lib/zoobot.sh" __delayed_start >/dev/null 2>&1 &
            stop_daemon
            return
        fi
    fi
...
status_daemon() {
    ...
    if session_exists; then
        ...
        echo "  Start: zoobot start"
    fi

    echo ""

    # Channel process status
    local ready_file="$ZOOBOT_HOME/channels/whatsapp_ready"
