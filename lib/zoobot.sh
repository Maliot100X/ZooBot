...
# ZooBot - Main daemon using tmux + claude -c -p + messaging channels
#
...
# ZOOBOT_HOME = data directory (settings, queue, logs, etc.)
# Always defaults to ~/.zoobot; override via ZOOBOT_HOME env var.
ZOOBOT_HOME="${ZOOBOT_HOME:-$HOME/.zoobot}"

TMUX_SESSION="zoobot"
LOG_DIR="$ZOOBOT_HOME/logs"
SETTINGS_FILE="$ZOOBOT_HOME/settings.json"
...
case "${1:-}" in
            ...
            visualize|viz)
                ...
                if [ ! -f "$SCRIPT_DIR/packages/visualizer/dist/team-visualizer.js" ] || \
                   [ "$SCRIPT_DIR/packages/visualizer/src/team-visualizer.tsx" -nt "$SCRIPT_DIR/packages/visualizer/dist/team-visualizer.js" ]; then
                    ...
                    if ! (cd "$SCRIPT_DIR" && npm run build -w @zoobot/visualizer 2>/dev/null); then
    ...
    update)
        ...
        ;;
    version|--version|-v|-V)
        echo "zoobot v$(get_current_version)"
        ;;
    *)
        local_names=$(IFS='|'; echo "${ALL_CHANNELS[*]}")
        show_banner
        ...
        echo ""
        echo "Commands:"
        echo "  start                    Start ZooBot"
        echo "  stop                     Stop all processes"
        echo "  restart                  Restart ZooBot"
